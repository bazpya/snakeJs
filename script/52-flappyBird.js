...
// That tells me the output will be somewhere between 0 and 1 (do you remember the sigmoid function?).
// Sounds like we have our activation function!
...
// Ok, so now we have our model and we can start creating our population of birds using the genetic algorithm.
...
const biasA = a.layers[0].bias.read(); //Remember tidy? We’re operating over the tensors we got in the crossover function.
...
trainPopulation = function (population) { return population.map(async model => { await model.fit(tf.tensor2d(model.history), tf.tensor1d(model.outputHistory), { shuffle: true, }); }); }

// Predicting the result from a model is not an async operation, but getting the output value is.
tf.tidy(() => { const outputs = this.Population[bird.index].predict(tf.tensor2d([inputs])); outputs.data().then(output => { if (output > 0.5) { bird.flap(); } }); })
